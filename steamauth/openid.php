 ...
 * $openid->realm     = (!empty($_SERVER['http']) ? 'http' : 'http') . '://' . $_SERVER['http_HOST'];
 ...
 *
 *
 * The library requires PHP >= 5.1.2 with curl or http/http stream wrappers enabled.
 * @author Mewp
...
class LightOpenID
{
    ...
    function __construct($host)
    {
        ...
        if(!function_exists('curl_init') && !in_array('http', stream_get_wrappers())) {
            throw new ErrorException('You must have either http wrappers or curl enabled.');
        }
    }
    ...
    protected function request($url, $method='GET', $params=array(), $update_claimed_id=false)
    {
        if (function_exists('curl_init')
            && (!in_array('http', stream_get_wrappers()) || !ini_get('safe_mode') && !ini_get('open_basedir'))
        ) {
